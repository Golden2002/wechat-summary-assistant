...
系统提示词。程序内置 **19 套**模板，按 5 个场景分类：

| 分类 | 模板 | 适用场景 |
| --- | --- | --- |
| **通用** | 通用群聊总结（原版） | 话题混杂的普通群聊，也是未知场景的兜底（默认） |
| | **信息分享群 · 每日信息汇总** | **以转发、分享信息为主的群聊；按信息条目汇总，自动合并重复转发，每条保留来源与链接** |
...
### 信息分享类群聊：以「信息条目」为单位汇总

有一类群聊的主要用途是**分享与转发信息**，而不是讨论：成员各自丢进一条消息、一个链接、
一份文件，彼此之间并不形成对话。这类群聊的常见问题是同一条信息被多人先后转发，
如果按发言人逐条罗列，输出里会出现大量重复内容。

针对这类群聊，程序做了三层增强：

1. **新增专用模板「信息分享群 · 每日信息汇总」**。它以**信息条目**为单位组织输出，
   每条信息固定包含六项字段：

   | 字段 | 含义 |
   | --- | --- |
   | 【标题】 | 用一句话概括这条信息说的是什么 |
   | 内容 | 具体内容，保留关键数字、时间、版本号、地点与专有名词 |
   | 来源 | 谁发布或转发；转发时写明原始出处 |
   | 链路 | 记录里出现的链接、编号、文件名，原样保留 |
   | 形态 | 原创发布 / 转发 / 补充 / 提问，四选一 |
   | 提到人数 | 记录里有几个人提到过这条信息 |

   此外，该模板还单独设置了「同一条信息被多人提到」一节，要求把重复内容合并成一条，
   并写清各自补充了什么。

2. **四套信息密集的模板各自追加了一条汇总规则**：「医疗与AI信息交流群 · 每日行业日报」
   要求合并重复转发的行业消息；「学术科研交流群 · 文献选题与数据」要求把多人贴出的
   同一篇文献合并并把补充内容并入同一条目；「投资与财经讨论群 · 观点数据与风险提示」
   要求合并重复的数据，但**不合并**同一个人在不同时点给出的不同观点；
   「招聘求职与实习群 · 机会汇总」要求按岗位合并，而不是按发言人重复收录。

3. **跨场景的公共规则也补了两条**（写在 `SHARED_DISCIPLINE` 里，因此对全部模板、
   以及用户自建的模板都生效）：

   - 同一条信息被多人转发或重复提到时，合并成一条并注明「多人提到」；
     但同一个人在不同时点发布的不同信息之间**不要**合并；
   - 转发与转述的内容必须标明是转述，并尽量保留原始出处；不得把别人说的话
     写成转发者本人的观点。
...
**跨场景的公共约束**没有抄进 19 套模板，而是集中写在代码里（`SHARED_DISCIPLINE`），
